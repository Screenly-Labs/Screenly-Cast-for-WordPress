...
final class Request {
	...
	public static function server_string( string $key ): string {
		/*
		 * The narrowing has to precede unslashing and sanitising, because $_SERVER
		 ...
		 * access itself as unsanitised; sanitising happens three lines down. The
		 ...
		 */
		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash -- Narrowed below, then unslashed and sanitised.
		$raw = $_SERVER[ $key ] ?? '';
